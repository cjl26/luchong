...
public class UserController {
	...
	public String toListPage(HttpServletRequest request, Map<String, Object> model) {
		request.setAttribute("statusMap", TUser.initStatusMap());
		request.setAttribute("genderMap", TUser.initGenderMap());
		return "hicatcard/user/user-list";
	}
	...
	public String getCardListData(HttpServletRequest request) {
		...
		// 需要映射的实体类
		HqlParam hqlParam = WebUtil.buildHqlParam(paramMap, TUser.class, request);
		PageBean<TUser> cloudpayCardPage = hqlPageBeanHelper.findHqlPageBean(entityManager, hqlParam);
		
		AdminLteDataTableResp<TUser> resp = AdminLteUtil.pageBeanToAdminLteDataTableResp(cloudpayCardPage,request);
		return resp.toAdminLteJsonString(TUser.class);
	}
}
